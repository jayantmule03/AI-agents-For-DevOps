...
from argocd_monitor.argocd_temporal_agent import (
    ArgoCDMonitorWorkflow,
...
async def main():
    logger.info("=" * 60)
    logger.info("ArgoCD Monitor Temporal Worker")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(f"Creating worker for task queue: {ARGOCD_MONITOR_TASK_QUEUE}")
        worker = Worker(
            client,
            task_queue=ARGOCD_MONITOR_TASK_QUEUE,
            workflows=[ArgoCDMonitorWorkflow],
            activities=[
                get_application_status_activity,
                check_application_health_activity,
                get_application_logs_activity,
        ...
        logger.info("  - ArgoCDMonitorWorkflow")
        logger.info("")
        logger.info("Registered activities:")
        logger.info("  - get_application_status_activity")
        logger.info("  - check_application_health_activity")
        logger.info("  - get_application_logs_activity")
        logger.info("  - sync_application_activity")
        logger.info("  - get_application_resources_activity")
