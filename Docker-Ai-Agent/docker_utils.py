...
class DockerClientWrapper:
    ...
    def _container_to_info(self, container) -> ContainerInfo:
        ...
        # FIX: Handle cases where the image was deleted but the container still exists
        try:
            ...
            logger.warning(f"Could not retrieve image info for container {container.name}: {e}")
            # Fallback to the raw Image ID from attributes if available
            image_id = container.attrs.get('Image', 'unknown')
            image_name = f"missing-image ({image_id[:12]})"
